...
- 🔑 **Web API Schema Fetch (the reliable one):** Drop a free Steam Web API key into `Resources/Tools/steam_webapi_key.txt` and achievements/stats are pulled **directly by AppID** (`GetSchemaForGame`) — owner-independent, so it fixes niche games the top-owner scan can't find (and it's fast). Used first when present, owner scan as fallback. Get a key at steamcommunity.com/dev/apikey.
